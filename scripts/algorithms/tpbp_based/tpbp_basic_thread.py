            ...
            for m in range(div_cur + 1):
                if m >= self.t_div[orig_id, i]:
                    ...
                    if w in self.priority_nodes:
                        vals_n[orig_id, 0, i, m] += (self.discount_factor ** m) * self.lambda_priority * min(max(m * self.cf - future_visit_final[i], 0), self.time_periods[self.priority_nodes.index(w)])
